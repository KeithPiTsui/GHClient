...
public indirect enum Grid<Element> {
  ...
  case group(Grid<Element>, [String: Any])
  case section(Grid<Element>, [String: Any])
...
extension Grid {
  public func map<T>(_ transform: (Element) throws -> T) rethrows -> Grid<T> {
    ...
    case .group(let g, let attrs):
      return .group(try g.map(transform), attrs)
    case .section(let s, let attrs):
      return .section(try s.map(transform), attrs)
    }
  }
}
